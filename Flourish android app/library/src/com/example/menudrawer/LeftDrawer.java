...
public class LeftDrawer extends HorizontalDrawer {
    ...
    public void openMenu(boolean animate) {
    	Log.v("TAG", "openMenu  : "+  mMenuSize);
        //animateOffsetTo(mMenuSize, 0, animate);
    	
    	 animateOffsetTo((int)getResources().getDimension(R.dimen.left_drawer_width), 0, animate);
    	 
    	 Log.v("TAG", "Mobile width "+(int)getResources().getDimension(R.dimen.left_drawer_width));
    ...
    private void offsetMenu(int offsetPixels) {
        if (mOffsetMenu && mMenuSize != 0) {
            ...
            final float openRatio = 0;//(menuWidth - (float) offsetPixels) / menuWidth;
            Log.v("TAG", "menuWidth : "+menuWidth+" openRatio : "+openRatio+" offsetPixels : "+offsetPixels);
            if (USE_TRANSLATIONS) {
                if (offsetPixels > 0) {
